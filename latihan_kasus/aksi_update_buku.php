<?php
	...
	echo "<h2>Halaman Aksi UPDATE Buku</h2>";
	// deklarasikan variabel data
	$isbn_old; $isbn; $judul; $deskripsi;

	// periksa kode isbn lama sebagai acuan edit 
	if(isset($_POST["isbn_old"])){
		$isbn_old = $_POST["isbn_old"];
	}else{
		die("Gagal proses referensi ISBN acuan!<br />");
	}
	...
	*/
	 ...
	 if(update_buku($isbn_old, $isbn, $judul, $deskripsi, $pesan_error)){
	...
	function update_buku($isbn_old, $isbn, $judul, $deskripsi, &$info_error){
		include("config_db.php");
		$query = 'UPDATE tbuku SET isbn = :isbn, judul = :judul, deskripsi = :deskripsi  
				  WHERE isbn = :isbn_old;';
		...
		try {
		    ...
		    $stmt->bindParam(':isbn_old', $isbn_old);
